...
describe('0385_Triage_DI', () => {

    //Add the Triage element
    it('0385.1_To_Add_Triage' , () => {
      cy.get("#js-manage-elements-btn").should('be.visible').click()

      cy.get('body').then((body) => {
    
          if (body.find('#manage-elements-Triage[class="added"]').length == 0)
          {
    ...
    it('0385.2_To_Validate_Triage_SectionisAvailable' , () => {
    ...
    it('0385.4_To_Add_Details' , () => {
      ...
      if (clinicalmanagement.Triage_Set_Priority=="Immediate")
      {
        cy.get('[name="OEModule_OphCiExamination_models_Element_OphCiExamination_Triage[triage][priority_id]"]')
        .eq(0).click()
      }

      if (clinicalmanagement.Triage_Set_Priority=="Very urgent")
      {
        cy.get('[name="OEModule_OphCiExamination_models_Element_OphCiExamination_Triage[triage][priority_id]"]')
        .eq(1).click()
      }

      if (clinicalmanagement.Triage_Set_Priority=="Urgent")
      {
        cy.get('[name="OEModule_OphCiExamination_models_Element_OphCiExamination_Triage[triage][priority_id]"]')
        .eq(2).click()
      }

      if (clinicalmanagement.Triage_Set_Priority=="Standard")
      {
        cy.get('[name="OEModule_OphCiExamination_models_Element_OphCiExamination_Triage[triage][priority_id]"]')
        .eq(3).click()
      }

      if (clinicalmanagement.Triage_Set_Priority=="Low")
      {
        cy.get('[name="OEModule_OphCiExamination_models_Element_OphCiExamination_Triage[triage][priority_id]"]')
        .eq(4).click()
      }
      ...
      cy.get('#chief_complaint_id>div>ul>li').contains(clinicalmanagement.Triage_Chief_Complaint).click()
      cy.get('#eye_id>div>ul>li').contains(clinicalmanagement.Triage_Eye_Id).click()
