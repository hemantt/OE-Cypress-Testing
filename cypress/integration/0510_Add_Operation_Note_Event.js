...
describe('0510_Operation Note Event', () => {

//Test to check and click the Add Event button
  it('0510.1_Check_Add_Event_Button_And_Click', () => { 
  ...
  it('0510.2_Check_Add_Operation Note_Link_And_Click', () => { 
  ...
  it('0510.3_Check_Add_Operation_Note_Page_Loaded_Successfully', () => { 
  ...
  it('0510.4_Select An Operation Booking', () => { 
    ...
    if ($body.find('.booking-select').length <= 1) {}
      ...
      cy.fixture("operationnote.json").then((operationnote) => {
        ...
        cy.get('[data-element-type-name="Comments"] > header > h3').scrollIntoView().should('be.visible')
        ...
        cy.fixture("operationnote.json").then((operationnote) => {
          ...
          if (cntAnaesthetic_LA_Delivery_Methods == "Topical")
          {
            ...
            cy.get("input#Element_OphTrOperationnote_Anaesthetic__Topical").then(($checkbox) => {
              ...
              //If the checkbox is already checked, then do nothing. Otherwise, check the checkbox
              if(isChecked){}
              else
              {
